...
the YANG module capability URI format defined in ^RFC6020^.
...
Notifications specification ^RFC5277^.  The "ietf-restconf"
...
A new capability template called "queryparm" is defined to 
identify the specific revisions of each query parameter
supported by the server.

Each :queryparm capability URI has the following format:

    urn:ietf:params:restconf:capability:<name>:<version>

The "<name>" field is replaced with the name of the query parameter.
Vendors SHOULD use a prefix for query parameter names in order
to avoid collisions with other parameters.
The "<version>" field is replaced with the version number of the query.
...
!! row  ! point ! urn:ietf:params:restconf:capability:point:1.0
!! row  ! select ! urn:ietf:params:restconf:capability:select:1.0
!! row  ! start-time ! urn:ietf:params:restconf:capability:start-time:1.0
!! row  ! stop-time ! urn:ietf:params:restconf:capability:stop-time:1.0
...
If this query parameter is supported by the server, then the
"point" query parameter URI MUST be listed in the "capability" leaf-list
in ^mon-mod^. The "insert" query parameter MUST also be supported
by the server.
...
If this query parameter is supported by the server, then the
"start-time" query parameter URI MUST be listed in the "capability" leaf-list
...
If this query parameter is supported by the server, then the
"stop-time" query parameter URI MUST be listed in the "capability" leaf-list
...
resource, as described in ^path-resolution^.  All
of the examples in this document assume "/restconf" as the
discovered RESTCONF API root path.  The URI template ^RFC6570^
...
JSON encoding rules are defined in ^I-D.lhotka-netmod-json^.
...
JSON encoding of meta-data is defined in ^I-D.lhotka-netmod-json^.
...
The solution preserves aspects of NETCONF Event Notifications ^RFC5277^
while utilizing the Server-Sent Events ^wd-eventsource^ transport strategy.
...
a RESTCONF server using the GET operation on the 
"restconf-state/streams" data resource defined in ^mon-mod^.
...
!! head ! Name ! Description
!! row  ! start-time ! replay event start time
!! row  ! stop-time ! replay event stop time
!! row  ! filter ! boolean content filter

The semantics and syntax for these query parameters are
defined in the "query-parameters" YANG grouping in ^module^.
...
!! include-figure ietf-restconf.yang extract-to="ietf-restconf@2014-10-07.yang"
...
!! include-figure ietf-restconf-monitoring.yang extract-to="ietf-restconf-monitoring@2014-10-07.yang"
...
!! include-figure ietf-yang-library.yang extract-to="ietf-yang-library@2014-10-07.yang"
...
- moved 'modules' to new YANG module (ietf-yang-library)
...
Status: closed-pending; Callhome will not support RESTCONF at this time.
The exact requirements for extracting user identity and session
information from TLS is still TBD. (Issue #9 on the issue tracker)

** get-bulk query parameters
...
The work will not be part of RESTCONF. An additional RFC
will be needed to enhance the basic retrieval features.
...
This work will not be part of RESTCONF. The "with-defaults"
capability from RFC 6243 may be adapted as a query parameter.
The server should advertisre its basic mode in the capabilities list.
...
Status: proposal added

Update-02:


The capabilities container has been added to the ietf-restconf-monitoring
module

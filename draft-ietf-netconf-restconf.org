...
a "merge" operation when using a plain patch (see ^patch^),
...
This draft defines a plain patching mechanism using the application/yang.data media-type.  Please see ^draft-ietf-netconf-yang-patch-02^ for an alternate media-type supporting more granular control.

...
specific datastore contents, protocol operations, and notification events.
...
is always a mirror of the NETCONF running datastore,
if the server also supports NETCONF.
...
query parameters and protocol features supported by the server.

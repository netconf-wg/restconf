...
It is invoked using a POST method on the operation resource.  An
RPC operation is invoked as:
...
"*" is used to select all child nodes of a node.
For example, to retrieve all the child nodes of the 'admin' container,
 use: "fields=admin(*)".

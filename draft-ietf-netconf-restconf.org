...
prefix (such ass the enterprise or organization name) for query
parameter names in order to avoid collisions with other parameters.

*** Query Parameter URIs

A new set of capabilities are defined to identify the specific
revisions of each query parameter supported by the server.

Each such capability URI has the following format:
...
the "start-time" query parameter for that stream.
...
If the "replay-support" leaf is present in the "stream"
entry (defined in ^mon-mod^) then the server MUST support
the "stop-time" query parameter for that stream.
